    ...
    <div class="container mx-auto px-4">
        <div class="bg-white rounded-xl shadow-lg p-8 mb-8">
            <div class="flex justify-between items-center">
                <h1 class="text-4xl font-bold text-gray-800 mb-2">Modifier le Menu</h1>
        ...
        <div class="bg-white rounded-xl shadow-lg p-8">
            <form action="{{ route('menus.update', $menu->id_menu) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="nom" value="{{ $menu->nom }}" required
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" rows="4"
                                  class="w-full p-4 border-2 border-gray-300 rounded-lg focus:border-green-500 focus:ring-2 focus:ring-green-200 transition-all duration-200">{{ $menu->description }}</textarea>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Catégorie</label>
                        <input type="text" name="categorie" value="{{ $menu->categorie }}" required
                               class="w-full p-4 border-2 border-gray-300 rounded-lg focus:border-green-500 focus:ring-2 focus:ring-green-200 transition-all duration-200">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="disponible"
                                class="w-full p-4 border-2 border-gray-300 rounded-lg focus:border-green-500 focus:ring-2 focus:ring-green-200 transition-all duration-200">
                            <option value="1" {{ $menu->disponible ? 'selected' : '' }}>Oui</option>
                            <option value="0" {{ !$menu->disponible ? 'selected' : '' }}>Non</option>
                        </select>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select name="id_restaurant" required
                            ...
                            <option value="{{ $restaurant->id_restaurant }}" {{ $menu->id_restaurant == $restaurant->id_restaurant ? 'selected' : '' }}>
                                {{ $restaurant->nom }}
                            </option>
                            @endforeach
                        </select>
